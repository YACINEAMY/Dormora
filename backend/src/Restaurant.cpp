...
QString optionalString(const QJsonObject &object, const char *field, const char *legacyField)
{
    const QJsonValue value = object.value(field);
    if (value.isString()) {
        return value.toString();
    }
    // Older saved files used "ImagePath" names before the JSON fields were
    // standardized as "ImageUrl". Accept both so old data still loads.
    const QJsonValue legacyValue = object.value(legacyField);
    return legacyValue.isString() ? legacyValue.toString() : QString();
}
...
DailyMenu DailyMenu::fromJson(const QJsonObject &object)
{
    return {
        ...
        optionalString(object, "breakfastImageUrl", "breakfastImagePath"),
        optionalString(object, "lunchImageUrl", "lunchImagePath"),
        optionalString(object, "dinnerImageUrl", "dinnerImagePath"),
    };
}
...
QJsonObject Restaurant::toJson() const
{
    ...
    // QHash stores meal counts efficiently; sorting dates gives stable JSON.
