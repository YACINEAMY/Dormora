...
QVector<Student> University::students() const
{
    ...
    // QHash iteration order is not stable, so sort lists before showing,
    // testing, or saving them.
...
QVector<Dormitory> University::dormitories() const
{
    ...
    // Stable ordering keeps JSON output and GUI tables predictable.
...
{
    // All room assignments pass through the dormitory aggregate so room
    // membership and the student's stored assignment change together.
    Student &targetStudent = student(studentId);
...
std::optional<DailyMenu> University::restaurantMenuForStudent(const QString &studentId, const QDate &date) const
{
    ...
    // A resident can only access the restaurant attached to their dormitory.
...
{
    // Persistence can be edited outside the app, so loaded data is checked from
    // both directions: room membership and each student's assignment.
